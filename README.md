...
### デモモード (キー不要・まず試すとき)
- AIキーを何も設定していない場合、自動的に**デモモード**で動作します
- AI分析・コンテンツ生成が、実際のAPIを呼ばずに決定的なサンプル出力を返します
- **Supabaseだけ設定すれば、AIキー無しで「登録→分析→パッケージ生成→数値記録」の一連の流れをクリックして確認できます**
- サンプル出力には「【デモ出力】」マーカーが付き、設定画面にも「デモモード」と表示されます
- 本物のAIを使う準備ができたら、下記のGeminiキーを設定するだけです
...
プロバイダを変更する場合は `.env.local` の `AI_PROVIDER` を修正して `npm run dev` を再起動してください。選択したプロバイダのキーが空なら自動でデモモードにフォールバックします。
